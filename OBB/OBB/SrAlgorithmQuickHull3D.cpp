...
bool SrAlgorithmQuickHull3D::collinear(const Point&p0, const Point& p1,const Point& p2)
{
	Vector normal = (p1 - p0).cross(p2 - p0);
	double eps = 0.000001;

	if( normal.magnitudeSquared()<eps )
		return true;
	return false;
}

bool SrAlgorithmQuickHull3D::coplanar(const Point&p0, const Point& p1,const Point& p2,const Point& p3)
{
	SrVector3 normal = (p1 - p0).cross(p2 - p0);
	float result = normal.dot(p3 - p0);
	double eps = 0.000001;
	if( abs(result)<eps )
		return true;
	return false;
...
void SrAlgorithmQuickHull3D::deallocate(FacetList& ftList)
{
...
void SrAlgorithmQuickHull3D::findVisibleFacet(cVertex* furPoint,cFacet* f,FacetList& visibleSet,BoundaryEdgeMap& boudaryMap)
{
	...
	for( ; facetIter!=visibleSet.end(); facetIter++ )
	{
		for( i=0 ; i<3 ; i++ )
		{
			cFacet* neighbor = (*facetIter)->mNeighbors[i];
...
void SrAlgorithmQuickHull3D::constructNewFacets(cVertex* point,FacetList& newFacetList,BoundaryEdgeMap& boundary)
{
	...
	do
	{
		...
		edge->neighbors[1]->mVisitFlag = FACET_NULL;
		mBorderNum --;
		//Update neighbor facet of the invisible facet , at least one edge of which belong to the boundary.
		for( i=0 ; i<3 ; i++ )
		{
			if( edge->neighbors[1]->mNeighbors[i] == edge->neighbors[0] )
			{
				ASSERT( edge->neighbors[1]->mNeighbors[i]->mVisitFlag == FACET_VISITED);
				edge->neighbors[1]->mNeighbors[i] = facet;
				break;
			}
		}
		facet->mVertex[0]	 = point;
		facet->mVertex[1]	 = edge->point[0];
		facet->mVertex[2]	 = edge->point[1];
		facet->mNeighbors[1] = edge->neighbors[1];
		newFacetList.push_back(facet);
		boundary.erase(current);
		current = boundary.find(edge->point[1]);
		delete edge;
	...
	{
		(*curFacet)->mNeighbors[0] = *lastFacet;
		(*lastFacet)->mNeighbors[2]= *curFacet;
	}


}

void SrAlgorithmQuickHull3D::determineOutsideSet(FacetList& facetList, VertexList& allVertex)
{
...
void SrAlgorithmQuickHull3D::updateFacetPendList(FacetList& facetPendList , FacetList& newFacetList,cFacet*& head)
{
...
void SrAlgorithmQuickHull3D::partitionOutsideSet(FacetList& facetPendList,FacetList& newFacetList,VertexList& allVertex,cFacet*& head)
{
	determineOutsideSet(newFacetList,allVertex);
	updateFacetPendList(facetPendList,newFacetList,head);
}

void SrAlgorithmQuickHull3D::gatherOutsideSet(FacetList& facetPendList, FacetList& visFacetList,VertexList& visOutsideSet)
{
	FacetIterator facetIter;
	FacetIterator tempFacetIter;
...
void SrAlgorithmQuickHull3D::quickHullScan(FacetList& facetPendList,cFacet*& head)
{
...
bool SrAlgorithmQuickHull3D::initTetrahedron(VertexList& vertexList,FacetList& hull)
{
	//Check weather or not all the vertexes are collinear.
	VertexIterator ptIndex1 = vertexList.begin();
	VertexIterator ptIndex2 = ptIndex1;
	VertexIterator ptIndex3 = vertexList.end();
	ptIndex2 ++;
	ptIndex3 --;
	...
	// in the y-dimension.
	VertexIterator minx = vertexList.begin(), maxx = vertexList.begin(), miny = vertexList.begin();
	VertexIterator vertexIter = vertexList.begin();
	for( ; vertexIter!=vertexList.end(); vertexIter ++ )
	{
	...
	float minDist = plane.distance((*vertexList.begin())->mPoint);
	float maxDist = minDist;
	vertexIter = vertexList.begin();
	VertexIterator  minPtIndex = vertexIter, maxPtIndex = vertexIter;
	vertexIter++;
	for(  ; vertexIter!=vertexList.end() ; vertexIter++ )
	{
		float dist = plane.distance((*vertexIter)->mPoint);
		if( dist<minDist )
		{
			minDist = dist;
	...
	else
	{
		...
		vertexList.erase(ptIndex1);
		vertexList.erase(ptIndex2);
		vertexList.erase(ptIndex3);
		vertexList.erase(extIndex);
...
void SrAlgorithmQuickHull3D::recursiveExport(cFacet* head,std::map<cVertex*,int>& vMap,int& vertexId,std::list<cFacet*>& fList)
{
	...
	for( i=0 ; i<3 ; i++ )
	{
		...
		{
			vMap.insert(std::make_pair(head->mVertex[i] , vertexId++));
		}
	}
	for( i=0 ; i<3 ; i++ )
	{
		if( head->mNeighbors[i]->mVisitFlag != FACET_VISITED )
		{
			recursiveExport(head->mNeighbors[i],vMap,vertexId,fList);
		}
	}
}


void SrAlgorithmQuickHull3D::exportHull(cFacet* head, tHull* hull)
{
	std::map<cVertex*,int> vMap;
	std::list<cFacet*> fList;
	int vertexId = 0;
	recursiveExport(head,vMap,vertexId,fList);

	hull->numVertex = vMap.size();
	hull->vertex	= new SrPoint3D[hull->numVertex];

	std::map<cVertex*,int>::iterator mapIter;
	for( mapIter = vMap.begin() ; mapIter!=vMap.end() ; mapIter ++ )
	{
		ASSERT(mapIter->second<hull->numVertex);
		hull->vertex[ mapIter->second ].x = mapIter->first->mPoint.x;
		hull->vertex[ mapIter->second ].y = mapIter->first->mPoint.y;
		hull->vertex[ mapIter->second ].z = mapIter->first->mPoint.z;
	}

	hull->numFacet = fList.size();
	hull->facet = new tFacet[ hull->numFacet ];
	std::list<cFacet*>::iterator listIter;
	int v0, v1, v2;
	int facetIndex = 0;
	cFacet* tempFacet;
	for( listIter = fList.begin() ; listIter!=fList.end() ;  )
	{
		v0 = vMap[(*listIter)->mVertex[0]];
		v1 = vMap[(*listIter)->mVertex[1]];
		v2 = vMap[(*listIter)->mVertex[2]];
		ASSERT(v0<hull->numVertex);
		ASSERT(v1<hull->numVertex);
		ASSERT(v2<hull->numVertex);

		hull->facet[facetIndex].vertexIndex[0] = v0;
		hull->facet[facetIndex].vertexIndex[1] = v1;
		hull->facet[facetIndex].vertexIndex[2] = v2;
		facetIndex ++;
...
bool SrAlgorithmQuickHull3D::quickHull(SrPoint3D* points, int numPoint, tHull* resultHull)
{
	...
	determineOutsideSet(newFacetList,vertexList);
